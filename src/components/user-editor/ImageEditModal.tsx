...
import { Upload, RotateCw, Trash2, Palette, X, Move } from 'lucide-react';
...
export function ImageEditModal({ image, position, onUpdate, onDelete, onClose }: ImageEditModalProps) {
  ...
  const initialX = typeof window !== 'undefined' ? (window.innerWidth - 600) / 2 : 50;
  const initialY = typeof window !== 'undefined' ? (window.innerHeight - 800) / 2 : 100;
  ...
  return (
    <div 
      className="fixed bg-white rounded-lg shadow-2xl border border-gray-200 flex flex-col w-[600px] max-h-[90vh] overflow-hidden z-[100]"
      style={{
        left: `${modalPos.x}px`,
        top: `${modalPos.y}px`,
        transform: 'none',
    ...
    >
      <div 
        className="flex items-center justify-between px-6 py-4 border-b bg-gray-50/50 cursor-move"
